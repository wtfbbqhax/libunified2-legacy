...
void Unified2PrintExtraRecord(Unified2Extra *extra)
{
    printf("Sensor id           %d\n", extra->sensor_id);
    printf("Event id            %d\n", extra->event_id);
    printf("Event second        %d\n", extra->event_second);
    printf("Data Type           %s\n", (extra->data_type == EVENT_DATA_TYPE_BLOB ? "BLOB" : "UNKNOWN" ));
    printf("Info                %s\n", 
        extra->type == EVENT_INFO_XFF_IPV4 ? "IPv4 XFF" :
        extra->type == EVENT_INFO_XFF_IPV6 ? "IPv6 XFF" : 
        extra->type == EVENT_INFO_REVIEWED_BY ? "Reviewed By" :
        extra->type == EVENT_INFO_GZIP_DATA ? "GZIP data" : "Uknown" );
...
HRESULT Unified2PrintRecord(Unified2Entry *entry) {
    ...
    switch( entry->record->type )
    {
        ...
        printf("\n__ Event ______________________________________________________________\n");
        Unified2PrintEventRecord(entry->event);
        break;

        case UNIFIED2_IDS_EVENT_V2:
        printf("\n__ Event v2 ___________________________________________________________\n");
        Unified2PrintEventv2Record(entry->event_v2);
        break;

        case UNIFIED2_IDS_EVENT_IPV6:
        printf("\n__ Event6 _____________________________________________________________\n");
        Unified2PrintEvent6Record(entry->event6);
        break;

        case UNIFIED2_IDS_EVENT_IPV6_V2:
        printf("\n__ Event6 v2 __________________________________________________________\n");
        Unified2PrintEvent6v2Record(entry->event6_v2);
        break;

        case UNIFIED2_PACKET:
        printf("\n__ Packet _____________________________________________________________\n");
        ...
        printf("\n__ Extra ______________________________________________________________\n");
        Unified2PrintExtraRecord(entry->extra);
        printf("\n");
        Unified2PrintPacketData(entry->extra_data, entry->extra->extra_length);
        break;
    }
