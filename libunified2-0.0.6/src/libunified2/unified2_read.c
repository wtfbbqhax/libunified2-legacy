...
/* Function: Unified2ReadExtra
 *
 * Purpose: Parse Unified2 Extra Data
 *
 * Arguements:
 *      Unified2 *
 *
 ...
 *      void *
 */
Unified2Extra * Unified2ReadExtra(Unified2 *u2) {
    Unified2Extra *extra;
    int bytes_read;
    ...
    extra = (Unified2Extra *)malloc(sizeof(Unified2Extra));
    if(extra == NULL)
    {
        return NULL;
    }

    bytes_read = Unified2Read(u2, extra, sizeof(Unified2Extra));

    if(!bytes_read || bytes_read != sizeof(Unified2Extra))
    {
        return NULL;
    }

    extra->sensor_id = ntohl(extra->sensor_id);
    extra->event_id = ntohl(extra->event_id);
    extra->event_second = ntohl(extra->event_second);
    extra->type = ntohl(extra->type);
    extra->data_type = ntohl(extra->data_type);
    extra->extra_length = ntohl(extra->extra_length);

    return extra;
}

/* Function: Unified2ReadExtraData
 *
 * Purpose: Parse Unified2 Extra Data
 *
 * Arguements:
 ...
 *      void *
 */
void * Unified2ReadExtraData(Unified2 *u2, Unified2Extra *extra) {
    int bytes_read;
    void *extra_data;
    ...
    extra_data = (void *)malloc(extra->extra_length);
    bytes_read = Unified2Read(u2, extra_data, extra->extra_length);

    if(!bytes_read || bytes_read != extra->extra_length)
    {
        return NULL;
    }
...
HRESULT Unified2ReadNextEntry(Unified2 *u2, Unified2Entry *entry) {
    ...
    switch( entry->record->type )
    {
        ...
        case UNIFIED2_EXTRA_DATA:
            entry->extra = Unified2ReadExtra(u2);
            entry->extra_data = Unified2ReadExtraData(u2, entry->extra);
            if( entry->extra == NULL )
                return UNIFIED2_ERROR;
            break;
